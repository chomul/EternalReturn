...
### 1.2 구현 방안이 여럿이면 — `Docs/4_Argument/` 에 비교 문서를 만든다

세부 구현에 **선택 가능한 방안이 2개 이상**이면 채팅으로 설명하지 말고, **문서를 만들어서 사용자가 고르게 한다.**

- 위치·이름: `Docs/4_Argument/N. 주제.md` — `N`은 1부터 증가하는 일련번호
  (예: `Docs/4_Argument/1_스킬시스템_구조선택.md`)
...
5. ⭐ **근거와 출처** — **모든 주장에 출처를 단다. 출처 없는 주장은 쓰지 않는다.**
   ...
   - 공식 문서: URL + 문서 제목
   - 이 프로젝트 문서: `Docs/0_GameDesign/...` 상대 경로 + 섹션 번호
...
### 원인을 찾은 문제는 `Docs/5_ErrorReport/` 에 기록한다

**빌드 에러, 조용히 안 먹는 설정, 설계 오류를 고쳤으면 기록한다.** 고치고 끝내지 않는다.

- 위치·이름: **`Docs/5_ErrorReport/E##_짧은제목.md`** — 번호는 1부터 증가
- 목록: `Docs/5_ErrorReport/00_에러목록.md` 에 한 줄 추가 (최신이 위)
- 작성 시점: **고친 직후.** 다음 작업으로 넘어가기 전에 쓴다

**반드시 포함할 것**

1. **증상** — 무엇이 어떻게 잘못 보였나. 에러 메시지는 **원문 그대로**
2. **원인** — ⭐ **근거를 단다.** 엔진 소스는 `파일:라인`, 설정은 항목명, 로그는 원문 인용
3. **해결** — 무엇을 어떻게 고쳤나
...
- ⭐ **원인을 못 찾았으면 `(원인 미확인)` 이라고 적는다.** 추측을 원인처럼 쓰지 않는다.
  부분만 밝혀졌으면 어디까지 확인했는지 명시한다
- 기록 대상은 **코드 개발 중 발생한, 원인을 찾은 문제**다
  | 적는다 | 적지 않는다 |
  |---|---|
...
- ⭐ **특히 "빌드는 통과하는데 동작하지 않는" 종류를 반드시 남긴다.**
  이런 것은 기록이 없으면 몇 달 뒤에 똑같이 하루를 쓴다

---

### 기능 작업이 끝나면 — `Docs/3_EditorTasks/` 문서를 **반드시** 만든다

C++ 빌드가 통과했다고 기능이 끝난 게 아니다. **에디터에서 손대야 할 것과 PIE 로 확인해야 할 것을 문서로 남긴다.**

- 위치·이름: **`Docs/3_EditorTasks/F##_기능명.md`** — Task 폴더와 **같은 이름**을 쓴다
  (예: `Docs/1_Task/F01_코어골격_팀ID/` → `Docs/3_EditorTasks/F01_코어골격_팀ID.md`)
...
**이 프로젝트는 GAS(GameplayAbilitySystem) 위에서 만든다.** 2026-09-02 결정 — 근거는 `Docs/4_Argument/1_스킬시스템_구조선택.md`.
...
- **ASC 배치를 바꾸지 않는다.** 한 번 정하면 되돌리기 비용이 크다. 결정은 `Docs/1_Task/F00_GAS기반세팅/` 참조.
