...
### 빌드 후 — 에디터 작업이 남았다면 `.md` 문서로 남긴다

C++ 작업만으로 기능이 완성되지 않고 **에디터에서 수동으로 해야 할 일이 남으면**, 채팅으로만 설명하지 말고 **문서 파일을 만든다.**

- 위치·이름: `Docs/EditorTasks/YYYY-MM-DD_작업명.md` (예: `Docs/EditorTasks/2026-08-29_스킬쿨타임UI.md`)
- 작성 시점: 빌드가 통과한 **직후**. 빌드 실패 상태로 작업 문서를 만들지 않는다.
- 채팅에는 문서 경로와 3줄 요약만 남긴다. 긴 절차를 채팅에 복붙하지 않는다.

문서에 반드시 포함할 것:

1. **목적** — 이 작업을 왜 하는지, 끝나면 무엇이 동작하는지 한 문단.
2. **선행 조건** — 에디터 재시작 필요 여부, 필요한 플러그인, 먼저 끝나 있어야 할 작업.
3. **단계별 절차** — 번호 매긴 순서. 각 단계에 다음을 구체적으로 적는다.
   - 애셋 전체 경로 (`/Game/ERCharacter/BP_ERCharacter`)
   - 메뉴·패널 위치 (`Details 패널 > Stats 카테고리`)
   - 프로퍼티 **정확한 표시 이름**과 넣을 값 (C++ 변수명이 아니라 에디터에 보이는 이름)
   - 새로 만들 애셋은 타입·부모 클래스·이름·저장 위치까지 명시
4. **데이터 테이블 변경** — 해당하면 컬럼명과 예시 행을 표로. 행 이름 규칙도 적는다.
5. **검증 방법** — PIE에서 무엇을 눌러 무엇을 확인하면 성공인지. 멀티플레이 기능이면 **Net Mode / 플레이어 수** 설정까지 지정한다 (예: `Play As Client, Number of Players: 2`).
6. **주의점** — 흔한 실수, 값을 잘못 넣었을 때 나타나는 증상.

원칙: **사용자가 문서만 보고 그대로 따라 하면 끝나야 한다.** "적당히 설정하세요" 같은 문장은 쓰지 않는다. 값을 모르면 추측해서 적지 말고, 그 자리에서 사용자에게 묻는다.
