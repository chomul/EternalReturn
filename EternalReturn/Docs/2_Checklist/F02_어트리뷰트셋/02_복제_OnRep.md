...
- [x] ⭐ 서버에서 GE로 체력을 바꾸면 **클라 값이 갱신된다**
- [x] `OnRep_Health` 가 클라에서 호출된다
- [x] ⭐ `IncomingDamage` 가 클라에 **오지 않는다**
- [ ] 클라에서 어트리뷰트를 직접 바꿔도 서버 값이 안 변한다
  ⬜ **③ 절이 덮지 않는다.** 클라에서 값을 바꿀 수단이 없다 -
  `ER.Damage.SetStat` 은 서버가 아니면 거부한다(`IsOwnerActorAuthoritative`).
  ⭐ **그 거부 자체가 이 항목의 절반**이다. 나머지 절반(우회로 바꿨을 때)은
  치트 수단이 생기는 **F17 이후**에나 시험할 수 있다
